...
This document explains how to control what attendees see during large Microsoft Teams Town halls, so the right speaker and content are visible at the right time.

Teams active-speaker behaviour is not enough for a polished all-company event. Shared content, PowerPoint Live, attendee layouts, devices, and client differences can all affect what people see. The attendee view should therefore be managed deliberately using **Manage what attendees see**, rather than left to automatic speaker focus.
...
**Note on Teams Live Events:** Live Events should not be used for new all-company event designs in 2026. Microsoft has moved its strategic direction to Town halls and announced the retirement of Teams Live Events.

## Core feature: Manage what attendees see

Microsoft’s **Manage what attendees see in Microsoft Teams** feature lets organisers and production-tool controllers choose which presenters and shared content are visible to attendees.
...
In a Town hall, use **Production tools > Manage what attendees see**. Where available, choose **On with preview** so the producer can prepare presenters or shared content before attendees see them.
...
| **On with preview** | Lets the producer prepare the next presenter or content before attendees see it |
| **Send live** | Pushes the prepared presenter or content to the attendee view |
| **Pinning presenters** | Keeps key presenters easy to find during events with multiple speakers |
...
| Attendee cameras | Off, unless attendees are intentionally part of the visual experience |
| Attendee microphones | Off, unless audience participation is part of the format |
| Q&A/chat | Use only if it supports the event format and does not distract from visual control |
| Recording | Enable if the event needs to be available afterwards |

The most important controls for the attendee view are **Manage what attendees see**, **On with preview**, **Who can present**, and **Green room**.

## Admin centre settings to standardise

Use Teams admin settings and event policies to make the preferred setup repeatable.

| Objective | Admin recommendation |
|---|---|
| Make Town halls available | Enable Town halls for the users who organise all-company events |
| Avoid untrained use | Limit Town hall creation to trained organisers if appropriate |
...
The producer controls the attendee view and should normally be separate from the host.
...
- Use preview, where available, before selecting **Send live**.
- Prevent unexpected shares from changing the attendee view.
...
PowerPoint Live can be useful, but it may affect production control. Microsoft notes that when a PowerPoint Live session starts, the PowerPoint appears live to everyone immediately, which can bypass the preview-and-send-live workflow.

Choose the sharing method based on the desired attendee view, not presenter preference alone.
...
| Check | Done |
|---|---|
| Town hall created for the event |  |
...
Treat attendee view as something to be **produced**, not something Teams will always optimise automatically.
