...
⚬	In-Place Upgrade (Standard Path): If task volume is <3,000 and data is compatible, the plan converts directly in-place [cite: ]. All M365 Group access permissions are maintained [cite: ]. The original Basic plan enters a 90-day read-only archive; administrators can trigger a complete downgrade within this 90-day window if needed [cite: ].
⚬	Fallback Import Flow (Data Incompatibility Path): If data validation fails or custom fields conflict, the original Basic plan remains active and untouched [cite: ]. A copied Premium instance is generated for the initiating user only [cite: ]. The administrator must remediate the basic plan, re-assign team security roles, and complete cutover [cite: ].
...
⚬	Operation Set Staging: Workflow steps must:
	...
	3.	Call msdyn_ExecuteOperationSetV1 to process and commit changes to the schedule [cite: ].
Cost-Optimized Tiered Licensing Architecture
Assigning high-tier licenses to all 80 users is unnecessary [cite: ]. Deploy a hybrid model to minimize software spend:
...
⚬	License: Standard Microsoft 365 Base Edit Access (E3/E5) ($0 Incremental Cost) [cite: ].
⚬	Rights: Open shared Premium plans in Teams, mark tasks complete, update % completion, edit dates, add notes, and re-assign tasks [cite: ].
5-Phase Execution Roadmap
	...
	3.	Phase 3: Integration Staging: Re-author Power Automate flows using Project Schedule API Operation Sets; test dry-run upgrade in a sandbox environment [cite: ].
	4.	Phase 4: Production Cutover: Execute conversion during off-hours; verify Dataverse ingestion logs; switch Power Automate flow environments [cite: ].
...
⚬	Budget Efficiency: "We do not need to buy expensive software licenses for all 80 team members. By leveraging Microsoft's hybrid licensing model, over 80% of our team will update deliverables using our existing Microsoft 365 subscriptions at zero added software cost." [cite: ]
