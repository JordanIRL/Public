...
_As of May 2026 — cleaned copy with broken citation markers removed._
...
## 3. Explicit context: “Manage what attendees see”

Microsoft’s **Manage what attendees see in Microsoft Teams** guidance should be treated as a core source for this recommendation, not a secondary reference.
...
Key points from that Microsoft Support page:

- The feature is designed to let organisers and producers control what attendees see by bringing people and shared content **on screen** or **off screen**.
- In a **meeting**, the organiser should set **Who can present** to **Specific people**, select the presenters, then enable **Manage what attendees see** under **Production tools**.
- When this setting is enabled in a meeting, attendees do not have camera and microphone access.
- In a **webinar**, the setting is enabled under **Engagement**. Microsoft notes that attendee microphone and camera permissions are locked when the setting is applied.
- In a **town hall**, Microsoft says the setting defaults to managing what attendees see. Under **Production tools > Manage what attendees see**, organisers can choose **Off**, **On**, or **On with preview**.
- **On with preview** is the strongest fit for polished all-company calls because authorised production-tool controllers can queue presenters and content in a preview feed, then select **Send live** when ready.
- The **Live feed** represents what attendees are seeing.
- Presenters can be brought on screen and taken off screen from the **Manage screen** pane.
- The **LIVE** indicator in the Manage screen pane shows who is currently visible to attendees.
- Taking a presenter off screen does **not** automatically mute their microphone, so presenters must still mute themselves or be managed by the producer.
- For events with many presenters, producers can **pin** presenters in the Manage screen pane so the next speaker is easier to find.
- Shared content appears in the Manage screen pane and can be manually brought onto the live event feed.
- Important PowerPoint Live caveat: Microsoft says that if you start a PowerPoint Live session, the PowerPoint appears live to everyone immediately. For produced events, this means PowerPoint Live can bypass the preview-style staging workflow.
- Microsoft notes that presenter view for this experience is supported in Teams desktop and web, while Firefox and Safari attendees see the standard Teams meeting experience.

Implication for this project: this page directly supports the recommendation to solve the speaker-focus problem through **producer-led attendee view management**, especially **Town hall + Manage what attendees see + On with preview**, rather than relying on automatic active-speaker switching.
