...
## Device and room guidance

For high-profile presenters, use Teams-certified devices and rooms where possible. A known-good presenter setup is better than allowing every presenter to join from an untested laptop, headset, meeting room, or webcam.

Recommended device practices:

- Use a reliable headset, speakerphone, or room audio system.
- Test camera framing, lighting, and background before the event.
- Avoid switching devices immediately before going live.
- Use the same room, device, and account that will be used during the actual event.
- For leadership or host locations, consider a dedicated Teams Room or approved presentation setup.
- Keep a backup device or backup presenter option available for critical segments.
...
Useful Microsoft tools include Teams admin centre meeting quality data, Call Quality Dashboard, and Town hall insights where available.
...
Create or update Teams meeting and event policies for large internal events. Restrict who can create and run Town halls if needed, standardise presenter permissions, and keep PowerPoint Live available where appropriate.
