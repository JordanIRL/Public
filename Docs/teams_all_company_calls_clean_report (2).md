...
| Improve office viewing | Use Microsoft eCDN where many attendees join from the same physical sites |
| Improve production quality | Use 1080p Town hall only after validating network capacity and presenter devices |

## Suggested Teams PowerShell starting points

The exact commands should be validated against the current Microsoft Teams PowerShell module in a test policy before production use.

```powershell
# Review the current global meeting and event policy baselines
Get-CsTeamsMeetingPolicy -Identity Global
Get-CsTeamsMeetingConfiguration -Identity Global
Get-CsTeamsEventsPolicy -Identity Global

# Example presenter-focused meeting policy for large internal broadcasts
...
# Example events policy for internal all-company sessions
...
These examples are intended as a starting point, not a copy-and-paste production standard. Licensing, existing policy design, tenant configuration, and Teams PowerShell module version should all be checked first.
...
The presenter experience matters more than the average attendee experience. One presenter with poor audio or unstable screen sharing can affect the whole event.

Recommended network practices:

- Use wired connections for presenters where possible.
- Avoid routing real-time Teams media through VPN unless the network design has been validated for it.
- Implement Teams QoS where the network supports it.
- Validate office bandwidth before enabling higher-resolution Town halls.
- Use Microsoft eCDN if many attendees watch from the same corporate locations.
- Test from the actual rooms and networks that presenters will use.

Common Teams QoS values to validate with the network team:

| Workload | Source port range | DSCP |
|---|---:|---:|
| Audio | 50000–50019 | 46 |
| Video | 50020–50039 | 34 |
| App and screen sharing | 50040–50059 | 18 |

For high-profile presenters, use Teams-certified devices and rooms where possible. A known-good presenter setup is better than allowing every presenter to join from an untested laptop, headset, room, or network.

## Monitoring and review

After each event, review both technical quality and production quality.

Technical review should include:

- Presenter audio quality.
- Presenter video quality.
- Screen or app sharing quality.
- Network issues.
- Recording quality.
- Any device or room problems.

Production review should include:

- Whether the correct speaker was visible at the correct time.
- Whether shared content appeared cleanly.
- Whether any presenter went live too early or stayed live too long.
- Whether transitions were smooth.
- Whether Q&A or chat moderation worked.
- Whether the format should change next time.

Useful Microsoft tools include Teams admin centre meeting quality data, Call Quality Dashboard, Town hall insights where available, and Microsoft eCDN analytics where deployed.

## Rollout plan

### Phase 1: Agree the standard

Decide that company-wide calls are treated as produced events, not ordinary meetings. Confirm Town hall as the default format and agree the standard roles: organiser, co-organiser, producer, moderator, presenters, and IT support.

### Phase 2: Configure the tenant

Create or update Teams meeting and event policies for large internal events. Restrict who can create and run Town halls if needed, standardise presenter permissions, keep PowerPoint Live available, and define when Microsoft eCDN or 1080p Town hall should be used.

### Phase 3: Pilot the model

Run a pilot Town hall using the same people, rooms, devices, and network conditions expected for the real event. Test the green room, Manage what attendees see, presenter transitions, slide sharing, recording, Q&A, and attendee experience.

### Phase 4: Train organisers and presenters

Create a short runbook and checklist. Train organisers, producers, moderators, and presenters separately, because each role needs different instructions. Keep presenter guidance simple and focused on joining early, staying muted, waiting for cues, and not sharing unexpectedly.

### Phase 5: Review and improve

After each all-company call, review both technical quality and production quality. Update the checklist based on what actually happened, not just what the runbook expected.
...
9. Use certified devices and validated networks for presenters.
...
- Microsoft Learn: Microsoft eCDN — https://learn.microsoft.com/en-us/microsoftteams/streaming-ecdn-enterprise-content-delivery-network
- Microsoft Learn: Prepare your network for Teams — https://learn.microsoft.com/en-us/microsoftteams/prepare-network
- Microsoft Learn: QoS in Teams — https://learn.microsoft.com/en-us/microsoftteams/qos-in-teams
- Microsoft Learn: Monitor call quality and QoS — https://learn.microsoft.com/en-us/microsoftteams/monitor-call-quality-qos
- Microsoft Learn: Enable 1080p video resolution for Town hall — https://learn.microsoft.com/en-us/microsoftteams/enable-1080p-video-resolution-town-hall
